...
import type { InStoreHouseRequestData } from "@@/apis/store_houses/type"
...
import { inStoreHouseDataApi } from "@@/apis/store_houses"
...
const formData = ref<InStoreHouseRequestData>(cloneDeep({
  contract_number: "",
  item_number: "",
  in_store_count: 0
}))
...
function handle_in_store() {
  loading.value = true
  inStoreHouseDataApi(formData.value).then(() => {
    ...
    <el-card shadow="never">
      <div class="m-4">
        ...
        <el-form ref="formRef" :model="formData" label-width="100px" label-position="left">
          ...
          <el-form-item prop="in_store_count" label="入库数量" v-if="formData.item_number">
            <el-input v-model="formData.in_store_count" type="number" placeholder="请输入" />
          </el-form-item>
        </el-form>
